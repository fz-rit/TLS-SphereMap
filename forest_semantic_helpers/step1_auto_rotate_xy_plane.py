...
def rotate_original_points(original_df, rotation_matrix):
    """
    Replace original ground points with fewer sampled ground points and apply rotation.
    ...
    # Apply rotation to all points
    points = original_df[['X', 'Y', 'Z']].values
    rotated_points = apply_rotation(points, rotation_matrix)
    original_df[['X', 'Y', 'Z']] = rotated_points
...
def main():
    ...
    ground_sample_df_rotated = ground_df.copy()
    ...
    updated_df = rotate_original_points(sample_df, rotation_matrix)
    updated_output_path = output_dir / f"{pcd_path.stem}_rotated.csv"
    updated_df.to_csv(updated_output_path, index=False)
    
    # Save rotation matrix
    np.savetxt(output_dir / f"{pcd_path.stem}_rotation_matrix.txt", rotation_matrix)
