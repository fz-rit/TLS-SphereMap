...
Functions:
    ...
    interactive_visualize_pcd(all_points: np.ndarray, all_curvatures: np.ndarray, all_roughness: np.ndarray, curvature_radius: float, roughness_radius: float) -> None
        Creates an offscreen renderer for interactive visualization and captures snapshots of
        curvature and roughness color mappings.
...
def pad_neighbors(points: torch.Tensor, neighbors_list: List[torch.Tensor], max_neighbors: int) -> Tuple[torch.Tensor, torch.Tensor]:
...
def batch_neighborhood_search(points: torch.Tensor, radius: float, max_neighbors: int, device: str) -> List[torch.Tensor]:
...
def estimate_curvature_roughness_batched(points: np.ndarray, curvature_radius: float = 0.05, roughness_radius: float = 0.2, max_neighbors: int = 10) -> Tuple[np.ndarray, np.ndarray]:
...
def calculate_curvature_and_roughness(config: Dict[str, Any]) -> None:
    ...
    df_filtered = pd.DataFrame(pt_xyz_np, columns=['X', 'Y', 'Z'])
    ...
    if num_points > 30_000:
        ...
        df_grouped = df_filtered.groupby(pd.cut(df_filtered['X'], batch_num))
