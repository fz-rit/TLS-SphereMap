...
Functions:
    ...
    export_results(df_filtered: pd.DataFrame, all_curvatures: np.ndarray, all_roughness: np.ndarray, output_dir: Path, filename: Path) -> None
...
def faiss_knn(points: torch.Tensor, K: int, device: str = 'cuda') -> Tuple[torch.Tensor, torch.Tensor]:
    """Perform k-Nearest Neighbor search using FAISS with GPU support.

    Args:
        points (torch.Tensor): Tensor of points with shape (N, D).
        K (int): Number of nearest neighbors.
        device (str): Device for computation ('cuda' or 'cpu').

    Returns:
        Tuple[torch.Tensor, torch.Tensor]: Distances and indices of k-nearest neighbors.
    """
    points = points.cpu().numpy().astype('float32')  # FAISS requires float32 numpy array

    if device == 'cuda':
        res = faiss.StandardGpuResources()  # Initialize FAISS GPU resources
        index = faiss.IndexFlatL2(points.shape[1])  # L2 distance
        gpu_index = faiss.index_cpu_to_gpu(res, 0, index)  # Transfer to GPU
    else:
        gpu_index = faiss.IndexFlatL2(points.shape[1])  # CPU index
    
    gpu_index.add(points)  # Add points to index
    print(f"K= {K}-------------------")
    dists, indices = gpu_index.search(points, K)  # Perform k-NN search

    return torch.tensor(dists, device=device), torch.tensor(indices, device=device)
...
def calculate_neighbor_eigens(points: np.ndarray, 
                              K: int, 
                              device: str = 'cuda') -> Tuple[torch.Tensor, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    ...
    if not torch.cuda.is_available() and device.startswith("cuda"):
        raise RuntimeError("CUDA device specified, but no CUDA-enabled GPUs are available.")

    points_tensor = torch.tensor(points, dtype=torch.float32).contiguous().to(device)
    _, neighbor_indices = faiss_knn(points_tensor, K, device)
    ...
    centered_neighbors = (neighbors - centroids).contiguous()

    
    # Compute covariance matrices
    covariances = torch.matmul(centered_neighbors.transpose(1, 2), centered_neighbors) / (K - 1)
    
    # Eigen decomposition
    eigenvalues, eigenvectors = torch.linalg.eigh(covariances)
    
    return eigenvalues, eigenvectors, centered_neighbors


def estimate_curvature_roughness_batched(points: np.ndarray, 
                                         curvature_neighbors: int = 10, 
                                         roughness_neighbors: int = 20, 
                                         device: str = 'cuda') -> Tuple[np.ndarray, np.ndarray]:
    """Estimate curvature and roughness using k-Nearest Neighbors from PyTorch3D.

    Args:
        points (np.ndarray): Array of points.
        curvature_neighbors (int, optional): Number of neighbors for curvature estimation. Defaults to 10.
        roughness_neighbors (int, optional): Number of neighbors for roughness estimation. Defaults to 20.
    ...
    print("----------------------estimate_curvature_roughness_batched", device)
    curv_eigenval, _, _ = calculate_neighbor_eigens(points, curvature_neighbors, device)
    curvatures = curv_eigenval[:, 0] / curv_eigenval.sum(dim=1)

    # Roughness estimation
    _, rough_eigenvec, centered_neighbors = calculate_neighbor_eigens(points, roughness_neighbors, device)
    normal_vectors = rough_eigenvec[:, :, 0]
    roughness = torch.abs(torch.matmul(centered_neighbors, normal_vectors.unsqueeze(2)).squeeze(-1)).mean(dim=1)
...
def process_batches(dfs: List[pd.DataFrame], curvature_radius: float, roughness_radius: float, max_neighbors: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for df_batch in tqdm(dfs, desc="Processing Batches", unit="batch"):
        ...
        curvatures, roughness = estimate_curvature_roughness_batched(points)
        all_points.append(points)
        all_curvatures.append(curvatures)
        all_roughness.append(roughness)
    
    # Concatenate results and make them contiguous
    all_points = np.ascontiguousarray(np.vstack(all_points))
    all_curvatures = np.ascontiguousarray(np.hstack(all_curvatures))
    all_roughness = np.ascontiguousarray(np.hstack(all_roughness))
    return all_points, all_curvatures, all_roughness
...
def export_results(df_filtered: pd.DataFrame, all_curvatures: np.ndarray, all_roughness: np.ndarray, output_dir: Path, filename: Path) -> None:
...
def calculate_curvature_and_roughness(config: Dict[str, Any]) -> None:
    ...
    if num_points > 30_000:
        ...
        all_points, all_curvatures, all_roughness = process_batches(dfs, curvature_radius, roughness_radius)
    else:
